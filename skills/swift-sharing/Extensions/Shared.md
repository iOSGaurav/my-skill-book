...
[`appStorage`]([Shared Reader Key/app Storage( :store:)](SharedReaderKey/appStorage(_:store:).md)),
[`fileStorage`]([Shared Reader Key/file Storage( :decoder:encoder:)](SharedReaderKey/fileStorage(_:decoder:encoder:).md)), and
[`inMemory`]([Shared Reader Key/in Memory( :)](SharedReaderKey/inMemory(_:).md)). 

The [`appStorage`]([Shared Reader Key/app Storage( :store:)](SharedReaderKey/appStorage(_:store:).md)) strategy is useful for store small
...
The [`fileStorage`]([Shared Reader Key/file Storage( :decoder:encoder:)](SharedReaderKey/fileStorage(_:decoder:encoder:).md)) strategy is useful
...
And the [`inMemory`]([Shared Reader Key/in Memory( :)](SharedReaderKey/inMemory(_:).md)) strategy is useful for sharing any kind
...
See [Persistence strategies](../Articles/PersistenceStrategies.md) for more information on leveraging the persistence strategies that
...
[Observing changes to shared state](../Articles/ObservingChanges.md) for more information.
...
See [Testing](../Articles/Testing.md) for more information on how to test your features when using `@Shared`.
