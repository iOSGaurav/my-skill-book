...
that holds onto an integer that is stored in
 [`appStorage`]([Shared Reader Key/app Storage( :store:)](SharedReaderKey/appStorage(_:store:).md)) like so:
...
The same holds true for the [`fileStorage`]([Shared Reader Key/file Storage( :decoder:encoder:)](SharedReaderKey/fileStorage(_:decoder:encoder:).md))
and [`inMemory`]([Shared Reader Key/in Memory( :)](SharedReaderKey/inMemory(_:).md)) strategies. Even though those strategies do
