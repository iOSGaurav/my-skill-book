...
[`withLock`]([Shared/with Lock( :file ID:file Path:line:column:)](Shared/withLock(_:fileID:filePath:line:column:).md)) method in order to synchronize
...
### Potential race conditions in Now that we understand how technically `@AppStorage` is susceptible to race conditions, but that it
...
So this is why we require [`withLock`]([Shared/with Lock( :file ID:file Path:line:column:)](Shared/withLock(_:fileID:filePath:line:column:).md)) for
...
[`withLock`]([Shared/with Lock( :file ID:file Path:line:column:)](Shared/withLock(_:fileID:filePath:line:column:).md)) to mutate shared values. The
benefits of `@Shared` far outweighs this one single con. You get to use `@Shared`
[_anywhere_]([Use anywhere](../Extensions/Shared.md#Use-anywhere)) throughout your entire application, including UIKit and even
on non-Apple platforms. _And_ it works with more
[persistence strategies]([Persistence strategies](PersistenceStrategies.md)) than just user defaults. _And_ it is
well-behaved in previews and [tests]([Testing](Testing.md)), making it possible to write unit tests against
