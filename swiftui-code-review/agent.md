...
1. Collect the diff with the helper (pick the mode that matches the request):
   - `swiftui-code-review/scripts/collect-diff.sh --pr "<github-pr-url>"`
   - `swiftui-code-review/scripts/collect-diff.sh --branch <name>`
...
4. Deliver:
   - PR URL → `swiftui-code-review/scripts/post-review.sh --pr "<url>"
     --findings findings.json --event <REQUEST_CHANGES if any Blocker/High else COMMENT>`
   - Branch / current diff → `swiftui-code-review/scripts/render-report.sh
